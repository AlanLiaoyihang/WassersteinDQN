...
env_name_list = ['Breakout-ramNoFrameskip-v4']
...
#read from csv and with rolling mean data
def read(path, index, window = 1):
    """
    rolling mean for plottings
    @index: steprewards, stepvar, episodesrewards, episodesvar
    @window: window of rolling mean
    """
    a = pd.read_csv(path)
...
if __name__ == "__main__":
    for env_name in env_name_list:
        #list of rewards of different algorithm
        ...
        for i in range(len(name)):
            ...
            rs,rci,epsstats,epci = read(file_directories,index, window = 1)
        ...
        #plots steps stats
        for i in range(len(name)):
            plot_ci(np.array(rewards_steps[i]),np.array(ci_steps[i]),ax, interval = 2)
        ...
        for i in range(len(name)):
            plot_ci(np.array(rewards_episodic[i]),np.array(ci_episodic[i]),ax1, interval = 2)
